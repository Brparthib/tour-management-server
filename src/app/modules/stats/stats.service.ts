...
const sevenDaysAgo = new Date(now).setDate(now.getDate() - 7); // 7 days Ago
const thirtyDaysAgo = new Date(now).setDate(now.getDate() - 30); // 30 days Ago
...
const getTourStats = async () => {
  ...
  const totalTourByTourTypePromise = Tour.aggregate([
    // stage-1 : connect the TourType model - lookup stage
    {
      $lookup: {
        from: "tourtypes", // tourtypes - connected to local Tour model
        localField: "tourType", // tourType - field name in Tour model
        foreignField: "_id", // _id - connected by _id of TourType
        as: "type",
      },
    },
